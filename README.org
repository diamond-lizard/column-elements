...
** Definitions
For these definitions, let's assume the only text in a buffer or
narrowed region looks like this:
*** Diagram 1
#+BEGIN_EXAMPLE
...
delimiters (which is known as a *vertical gap*) or column 0, if the text
in that block extends left to include column 0.

The right boundary is the column containing the right-most
character in a block that's adjacent to a *vertical gap* or the
...
Blocks have not just left and right boundaries, but top and bottom
boundaries as well.

The top and bottom boundaries of a block are the lines of text
adjacent to a *horizontal gap* (a line that is either empty or which
...
at point.
**** Details
The *left* boundary of a given block is the column containing the
left-most non-delimiter character in the block adjacent to a vertical
gap or (if there is no vertical gap) it will be the left-most column
in the buffer.

The *right* boundary of a given block is the column containing the
right-most non-delimiter character in the block adjacent to a vertical
gap or (if there is no vertical gap) it will be the right-most column
in the buffer.

The *top* boundary of a given block is the line containing the
top-most non-delimiter character in the block adjacent ot a horizontal
gap or (if there is no horizontal gap) it will be the first line of
the buffer.

The *bottom* boundary of a given block is the line containing the
bottom-most non-delimiter character in the block adjacent ot a
horizontal gap or (if there is no horizontal gap) it will be the last
line of the buffer.
**** Example
#+BEGIN_EXAMPLE
foobar  hello
