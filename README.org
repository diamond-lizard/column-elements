...
#+END_EXAMPLE
*** Column elements, columns, column blocks, and column block boundaries
Diagram 1 shows text that the functions in this package will interpret
as 4 column blocks.

Each column block is delimited by certain characters.  In this
example, these delimiters are spaces.  (Which characters are used as
delimiters is configurable in using the
=column-elements--column-block-delimiters= variable)

Each column block starts has a left boundary column and a right
boundary column.

The left boundary column is the column containing the left-most
character in a column block that's adjacent to a column full of
delimiters (which is known as a *gap column*) or column 0, if the text
in that column block extends left to include column 0.

The right boundary column is the column containing the right-most
character in a column block that's adjacent to a *gap column* or the
right-most column in the buffer, if the text in that column block
extends right to include the right-most column.

The table below contains the boundaries for each of the column blocks
in Diagram 1.  The column blocks are numbered left-to-right from 1
to 4.

|--------------+---------------+----------------|
| Column block | Left boundary | Right boundary |
|--------------+---------------+----------------|
|            1 |             0 |             16 |
|            2 |            20 |             38 |
|            3 |            41 |             62 |
...
*** Column block rows and top and bottom boundaries
Column blocks have not just left and right boundaries, but top and bottom boundaries as well.
...
returns the left or right boundary of the column block at point.

If the argument is either ='top= or ='bottom=, then this function
returns the top or bottom boundary of the entire row of column blocks
at point.
**** Example
#+BEGIN_EXAMPLE
foobar  hello
