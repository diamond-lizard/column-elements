...
#+END_EXAMPLE
** Variables
*** column-elements--column-block-delimiter-char
The character to use as a delimiter of column blocks.
...
The default column block delimiter is a space character.
**** Example 1
#+BEGIN_EXAMPLE
one   two foo
three four
...
With this variable set to a space character, the above buffer would be considered to have two column blocks.  The first column block would contain the elements "one" and "three", while the second would contain the elements "two foo" and "four".
...
With this variable set to + (a single plus), the above buffer would be considered to have two column blocks.  The first column block would consist of two elements: "one++" and "three".  The second column block would also have two elements: "two  foo" and "four bar".
